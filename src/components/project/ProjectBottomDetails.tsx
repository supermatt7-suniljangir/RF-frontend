  ...
  return (
    <div className="md:w-2/3 w-full h-full">
      <div className='w-full flex flex-col'>
        <Card className={cn("rounded-none border-none h-full shadow-none w-auto py-0")}>
          <CardContent className="pt-6 pb-0 rounded-none border-none h-full shadow-none">
            <div className="flex flex-col items-center h-full">
              <LikeButton projectId={project._id} size="large" initialLikes={project.stats.likes || 0} />
        ...
        <div className='space-y-2 relative py-4'>
          ...
          <div className=' flex gap-2 justify-center flex-wrap'>
            {project.tools?.map((tool, index) => (
              <Card key={index} className='text-muted-foreground p-0 rounded-none'>
