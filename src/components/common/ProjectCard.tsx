    ...
    >
      <Link href={`/project/${project._id}`} className="w-full">
        <div className="relative h-64 overflow-hidden rounded-none">
          <Image
            unoptimized
            src={project?.thumbnail.url || Project2}
            alt={project.title}
            width={640}
            height={360}
          ...
          {(hovered || styles?.includes("pointer-events-none")) && (
            <CardHeader className="absolute w-full h-36 bottom-0 bg-gradient-to-t from-black via-black/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300">
      ...
      <CardContent className="px-4 py-1 mt-1">
        <div className="flex justify-between">
          ...
          {isOwner && isProfilePage && (
            <ProjectActionsContextMenu projectId={project._id} />
          )}
        </div>
      </CardContent>
    </Card>
