  ...
  const renderResults = () => {
    ...
    return (
      <div className="m-8 text-center">
        ...
        <p className="mt-2 text-lg">Try searching with popular tags</p>
      </div>
    );
  };
  ...
  return (
    <div className="flex-grow">
      {(projects?.length > 0 || users?.length > 0) && (
        <div className="relative ml-auto flex w-fit gap-4 px-4 py-4 md:gap-8 md:px-8">
          <Sort /> {(query || category || tag) && <Filters />}
        </div>
      )}
      <h2 className="my-2 text-center text-xl font-semibold">
        {type === "user"
          ...
          : query || category || tag
            ? `Projects matching "${query || category || tag}"`
            : "Featured"}
      </h2>
