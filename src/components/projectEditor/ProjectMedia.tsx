...
const ProjectMedia = () => {
  ...
  const handleRemoveMedia = (item: any) => {
    if (
      newMedia.find((m) => m.url === item.url) ||
      initialMedia.find((m) => m.url === item.url)
    ) {
      removeMedia(item);
    }
  };
  ...
  return (
    <div className="w-full">
      {media.map((item, index) => (
        <div key={index} className="relative p-2 md:p-0">
          <Button
            onClick={() => handleRemoveMedia(item)}
            variant="destructive"
            className="absolute right-2 top-2 z-10 h-auto w-auto p-2"
          >
